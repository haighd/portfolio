...
The `transform` function strips the raw MDX content and calculates reading time automatically. Every blog post gets a `readingTime` field (minimum 1 minute) without any manual input.
